...
class Ghost:
    ...
    def check_collisions(self):
        ...
        if 0 < self.center_x // 30 < 29:  # Back to original
        ...
        if self.direction == 0:
            ...
            if self.target[1] > self.y_pos and self.turns[3]:
                ...
                if self.target[0] > self.x_pos and self.turns[0]:
                    ...
                    self.y_pos += self.speed
        # Screen wrapping for ghosts - only through middle tunnel (row 15)
        if 360 < self.y_pos < 390:  # Tunnel area height range
            if self.x_pos < -30:
                self.x_pos = 870
            elif self.x_pos > 870:
                self.x_pos = -30
...
def draw_misc():
    ...
    if speed_boost:
        pygame.draw.circle(screen, 'yellow', (170, 930), 15)
    if invincibility:
        pygame.draw.circle(screen, 'red', (200, 930), 15)
    for i in range(lives):
        screen.blit(pygame.transform.scale(player_images[0], (30, 30)), (650 + i * 40, 915))
    if game_over:
        pygame.draw.rect(screen, 'white', [50, 200, 800, 300],0, 10)
        pygame.draw.rect(screen, 'dark gray', [70, 220, 760, 260], 0, 10)
        gameover_text_1 = font.render(f'Game over! Space bar to restart!', True, 'blue')
        gameover_text_2 = font.render(f'Score: {score}', True, 'blue')
        
        screen.blit(gameover_text_1, (100, 300))
        screen.blit(gameover_text_2, (100, 350))
    if game_won:
        pygame.draw.rect(screen, 'white', [50, 200, 800, 300],0, 10)
        pygame.draw.rect(screen, 'dark gray', [70, 220, 760, 260], 0, 10)
...
def check_collisions(scor, power, power_count, eaten_ghosts):
    global speed_boost, speed_counter, invincibility, invincibility_counter
    num1 = (HEIGHT - 50) // 32
    num2 = WIDTH // 30
    if 0 < player_x < 870:  # Back to original
        ...
        # Handle different tire types
        if level[center_y // num1][center_x // num2] == 'S':
            level[center_y // num1][center_x // num2] = 0
            scor += 50
            speed_boost = True
            speed_counter = 0
        if level[center_y // num1][center_x // num2] == 'M':
            level[center_y // num1][center_x // num2] = 0
            scor += 50
            # Medium tire gives balance - no special effect but good score
        
        if level[center_y // num1][center_x // num2] == 'H':
            level[center_y // num1][center_x // num2] = 0
            scor += 50
            invincibility = True
            invincibility_counter = 0
    ...
    for i in range(len(level)):
        for j in range(len(level[i])):
            ...
            if level[i][j] == 2 and not flicker:
                # Use tire image instead of circle for power pellets
                screen.blit(tire_h_img, (j * num2 + (0.5 * num2) - 10, i * num1 + (0.5 * num1) - 10))
            if level[i][j] == 3:
            ...
            if level[i][j] == 9:
                pygame.draw.line(screen, 'white', (j * num2, i * num1 + (0.5 * num1)),
                                 (j * num2 + num2, i * num1 + (0.5 * num1)), 3)
            # Add support for different tire types
            if level[i][j] == 'S':  # Speed boost tire
                screen.blit(tire_s_img, (j * num2 + (0.5 * num2) - 10, i * num1 + (0.5 * num1) - 10))
            if level[i][j] == 'M':  # Balance tire
                screen.blit(tire_m_img, (j * num2 + (0.5 * num2) - 10, i * num1 + (0.5 * num1) - 10))
            if level[i][j] == 'H':  # Invincibility tire
                screen.blit(tire_h_img, (j * num2 + (0.5 * num2) - 10, i * num1 + (0.5 * num1) - 10))
...
def draw_player():
    ...
    if direction == 0:
        screen.blit(player_images[0], (player_x, player_y))  # Right car
    elif direction == 1:
        screen.blit(player_images[1], (player_x, player_y))  # Left car
    elif direction == 2:
        screen.blit(player_images[2], (player_x, player_y))  # Up car
    elif direction == 3:
        screen.blit(player_images[3], (player_x, player_y))  # Down car
...
def check_position(centerx, centery):
    ...
    if centerx // 30 < 29:  # Back to original
        ...
        # Additional checks for smoother movement
...
while run:
    ...
    if speed_boost and speed_counter < 300:  # 5 seconds at 60fps
        speed_counter += 1
        player_speed = 4  # Double speed
    elif speed_boost and speed_counter >= 300:
        speed_counter = 0
        speed_boost = False
        player_speed = 2  # Normal speed
    
    # Handle invincibility power-up
    if invincibility and invincibility_counter < 600:  # 10 seconds at 60fps
        invincibility_counter += 1
    elif invincibility and invincibility_counter >= 600:
        invincibility_counter = 0
        invincibility = False
    ...
    if not powerup and not invincibility:
    ...
    # Screen wrapping - classic Pac-Man behavior through middle tunnel
    # Only allow wrapping when player is at the correct tunnel height (row 15 of the board)
    if 360 < player_y < 390:  # Tunnel area height range (row 15)
        if player_x > 740:  # Exit right side
            player_x = -47
        elif player_x < -50:  # Exit left side
            player_x = 740
